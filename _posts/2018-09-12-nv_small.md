...
**====Tips====** 
    ...
    1). Xilinx RAM资源的使用有4中方式---flexibility依次递减---i)源码推断, ii)XPMs, iii)直接例化RAM primitive, iv)例化RAM IP.
...
**====Tips====**
	...
	正常来说，添加如上信号便可以在BD工程中建立连接了，但是在仅作如上修改的情况下，在BD工程中必须要使用AXI smartconnect做PL和PS的接口互连，如果想使用AXI interconnect或干脆直接将NVDLA master与PS slave互连，还要进一步修改如下信号位宽,否则，BD工程在`validate`的时候会报错
